...
4. The learner continues through Task Response, Coherence and Cohesion, and Lexical Resource. Each round has a concrete paragraph-level revision prompt. The learner writes first, then compares with an authored example and may apply the example to the working essay.
5. The page keeps an original and a working version. A simple diff shows what changed. A correction matching an authored accepted variant can be applied directly; any other wording remains an unverified alternative. Criterion estimates change only after a verified authored improvement has actually been applied. The interface never awards a band increase merely for finding an issue or entering an unverified alternative.
6. The final view shows the two versions, criterion changes, found/missed issues, and a next-practice prompt. Restarting or switching essays discards the in-memory session with an explicit confirmation if edits exist.
