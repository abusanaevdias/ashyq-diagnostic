...
|---|---|---|---|---|
| LMS-001 | REVIEW (PR готов, ждёт merge) | Claude Opus 5 | ветка `claude/lms`, [PR #3](https://github.com/abusanaevdias/ashyq-diagnostic/pull/3); worktree `.claude/worktrees/ashyq-diagnostic-handoff-f946ca` | started 2026-09-13. Учебный слой по ТЗ пользователя; демо в `localStorage` `ashyq:v2:*` за интерфейсами `AuthAdapter`/репозиториев/`FileStorage` (Supabase встанет без правок UI). Роли student/teacher/author (admin зарезервирован), единая матрица прав `src/lib/lms/permissions.ts`; `/login` (email+пароль, демо-вход по ролям), `/me`, гвард `RequireRole` («Недостаточно прав» без редирект-петли); ученик — `/classes`, `/classes/[id]`, `/assignments/[id]` (текст + файлы ≤2 МБ/файл, 8 МБ/устройство, тред, aria-live); учитель — `/teacher`, `/teacher/classes/[id]`, `/teacher/{lessons,assignments}/new?class=`, `/submissions/[id]` (оценка + тред); автор — `/write`, `/write/[id]` (markdown-lite + превью, вкладки на mobile); `/blog` читает опубликованные посты из `BlogRepo`, новый `/blog/[slug]`; меню аватара в NavBar; сид (Айгерим К., Dias, Mepyat, Санжар; «IELTS Intermediate · Осень»); 6 событий аналитики. Ссылки-материалы только http(s), обложки постов только из `/public`. Коммиты: `0d2ae5a`, `dd736c2`, `fad508d`, `81c0a99`, `9dc4d52`, `f602cff`, merge `main` → `8dd3bd8`. Проверки на объединённом с `main` коде: lint, typecheck, bank 0/0, build; e2e 108/108 с `CRM_ADMIN_KEY`; `e2e:audio`, `check:crm`, `check:crm-ui`, `check:seo`, `card-image-check` PASS; `scripts/lms-e2e.ts` 69/69 (сценарии a–e на 1440/390, приватный режим, reduced-motion, события, grep хардкод-цветов, токены моушена ≤300ms); `lms-unit-check`, `seed-demo` PASS. Риски: данные демо живут на одном устройстве; пароли демо не хешируются (задокументировано в `auth.ts`); права проверяются только на клиенте — TODO(supabase)/RLS в `local-repos.ts`; смена заголовка опубликованного поста меняет slug. Для `SITE-SEARCH-001`: в `CleanUi.module.css` searchLink скрыт при ≤1180px и бургер с 1024px — место под слот аккаунта. Следующий safe step: Supabase-реализация `AuthAdapter`/репозиториев после решения `SEASON-AUTH-001` |
...
`LMS-001` (Claude Opus 5) — PR #3 готов к review; после merge строка переходит в DONE.
