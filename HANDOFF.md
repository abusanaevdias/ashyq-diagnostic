...
| FIX-AUDIO-CHECK-001 | IN_PROGRESS | Claude Opus 5 | `claude/fix-audio-check` (PR в `main`); worktree `.claude/worktrees/ashyq-diagnostic-handoff-f946ca` | started 2026-09-13; `scripts/audio-check.ts` зашит на `localhost:3000` и игнорирует `BASE_URL` (на prod-сборке не запускается, на dev виснет на `networkidle`) — перевести на `BASE_URL` и сверить селекторы с текущим quiz UI, чтобы `e2e:audio` из `CI-RELEASE-001` работал. Владею только `scripts/audio-check.ts`; `.github/**`, `docs/CI.md`, `package.json` не трогаю |
