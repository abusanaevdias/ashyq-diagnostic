...
|---|---|---|---|---|
| UX-FIXES-001 | IN_PROGRESS | Codex GPT-5 `/root` | `ai2/ux-fixes`; worktree `C:\Users\Dias\Documents\ChatGPT\ashyq-leads-durability` | started 2026-09-13; исправить находки SITE-AUDIT-001 (не блокированные на пользователя): (P1) `Reveal` — контент видим по умолчанию, скрывается только для элементов ниже первого экрана ПОСЛЕ гидрации (пустые зоны на мобильных / и /diagnostic); (P2) квиз: «Финиш» скрыт на первом вопросе (e2e кликает его после полного цикла — контракт «Завершить диагностику» не меняется), починить прозрачность фиксированного футера/шапки (текст вариантов читается сквозь бар — найти причину, сделать явно непрозрачным); (P2) «Контакты» в десктоп-шапке — добавить, если nav влезает на 920px (замерить); quick wins: `/diagnostic` в `SITE_ROUTES` + `CANONICAL_ROUTES` в seo-check (canonical на странице уже есть), быстрые ссылки в пустом состоянии `/search`. Owned: `src/components/ui/Reveal.tsx` + `.module.css`, `src/components/quiz/QuizRunner.tsx` (только footer/header-классы и условие «Финиш»), точечные строки `src/components/ui/CleanUi.tsx`, `src/lib/site.ts`, `scripts/seo-check.ts`, `src/components/SearchV3.tsx`, `scripts/e2e-check.ts` (только sitemap/поиск-блок). НЕ трогаю: `Landing.tsx`/`ProgramScreen.tsx` (A11Y-PERF-001), `globals.css`/`ReviewScreen`/tokens (V3-TOKEN-AUDIT-002), state/localStorage диагностики, aria-имена e2e. Цифры «12 000+/4.8/90%» НЕ трогаю — нужно подтверждение источника от пользователя |
| V3-TOKEN-AUDIT-002 | REVIEW (PR готов, ждёт merge) | Claude Opus 5 | ветка `claude/token-audit` (PR в `main`); worktree `.claude/worktrees/ashyq-diagnostic-handoff-f946ca` | started 2026-09-13; закрыть остаточный drift дизайн-системы v3 (`PRODUCT_BACKLOG.md`, P2): OG-картинка, PNG-карточка результата и `themeColor` берут цвета из одного источника `design/tokens.json` вместо дублей hex; success-state разбора (`bg-[#e8efe2]` в `ReviewScreen.tsx`) — на токен (новый semantic token только с согласия пользователя, иначе существующая палитра); остатки хардкодов в `globals.css` и `global-error.tsx`; `.21st/design.json` и `.21st/DESIGN.md` синхронизированы с v3; новый `scripts/token-audit.ts` — автоматический аудит неизвестных цветов/радиусов/шрифтов. Owned: `src/app/opengraph-image.tsx`, палитра в `src/lib/card-image.ts`, `themeColor` в `src/app/layout.tsx`, стили состояний в `src/components/review/ReviewScreen.tsx`, хардкод-цвета в `src/app/globals.css`, inline-цвета в `src/app/global-error.tsx`, `.21st/**`, `design/tokens.json` + `design/tokens.css` (новый semantic token success — утверждён пользователем 2026-09-13), цвета в `tailwind.config.ts`, новый `scripts/token-audit.ts`; shared — только эта строка в `HANDOFF.md` и строка задачи в `PRODUCT_BACKLOG.md`. Не трогаю `CleanUi.tsx` (`SITE-SEARCH-001`), `Landing.tsx`/`ProgramScreen.tsx`/`package.json`/CI (`A11Y-PERF-001`), бизнес-логику диагностики, scoring и тексты. Итог: токены `--success`/`--success-soft` (решение пользователя); `src/lib/design-tokens.ts` читает `tokens.json` для OG, PNG-карточки и `themeColor`; в `globals.css` имена v2 — алиасы токенов v3 прямо в `:root` (раньше вне `.v3` оставались цвета v2 — ссылки на `/contacts`, `/season`), хардкоды и радиусы — на токены, бумажное зерно убрано; `fonts.css` без Oswald/Playfair, 12 неиспользуемых woff2 удалены; точечно вне owned: `rounded-[22%]` → `--r-xs` в `Brand.tsx`, фолбэк `var(--r-md, 12px)` снят в `SearchV3.module.css`, сетке графика главной `fill: none` через `HomeV3.module.css` (сам `Landing.tsx` — зона `A11Y-PERF-001`, не тронут); `.21st/*` описывают v3. `scripts/token-audit.ts`: паритет `tokens.json` ↔ `tokens.css` и каналов `--c-*`, скан `src/`, рантайм 70 экранов (1440/390: публичные, LMS, квиз → результат → разбор), `SHOT_DIR` для снимков. До правок аудит нашёл 47 мест в исходниках и 3 в рантайме, после — 9/9. Проверки: lint, typecheck, bank 0/0, build; e2e 113/113; `lms-e2e` 69/69; audio, CRM, CRM UI, SEO (OG 60998 байт — как до правок), card PASS. Риски: каналы `--c-*` — ручная копия токенов (аудит сверяет); размеры и регистр `.display`/`.btn` старых экранов остаются слоем совместимости |
| A11Y-PERF-001 | IN_PROGRESS | Codex GPT-5 `/root` | `codex/a11y-perf`; worktree `C:\Users\Dias\Documents\ChatGPT\ashyq-a11y-perf` | started 2026-09-13; автоматический axe/Lighthouse gate для публичных маршрутов: WCAG violations, keyboard/focus, tap targets, overflow, reduced-motion и Performance ≥90; подключить отдельным CI step и сохранить отчёты как artifacts. Owned: новые `scripts/a11y-perf-check.ts`, `docs/A11Y_PERF.md`, `artifacts/a11y-perf/.gitkeep/.gitignore`; точечно `package.json`, `package-lock.json`, `.github/workflows/ci.yml`, `PRODUCT_BACKLOG.md`, `HANDOFF.md`. Первый audit доказал два независимых дефекта, поэтому scope расширен до точечных строк `src/components/Landing.tsx` (валидная семантика ARIA графика и согласование accessible/visible CTA name) и `src/components/ProgramScreen.tsx` (`main` landmark). Не трогать остальные app/components, `scripts/e2e-check.ts`, LMS, SITE-SEARCH-001 и файлы завершённого `ERROR-STATES-001` |
...
Активна `V3-TOKEN-AUDIT-002` (Claude Opus 5) — только её owned файлы, изменения через PR.
