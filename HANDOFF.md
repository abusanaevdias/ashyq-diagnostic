...
| FONT-CLS-001 | IN_PROGRESS | Claude Opus 5 | ветка `claude/font-cls` (PR в `main`); worktree `.claude/worktrees/ashyq-diagnostic-handoff-f946ca` | started 2026-09-14; убрать CLS от подмены шрифтов (Manrope 800 в hero `/season` ≈0.18): fallback-`@font-face` с `size-adjust`/`ascent-override` для Manrope и Inter, при необходимости preload. Owned: `src/app/fonts.css`, `src/app/layout.tsx` (только шрифты/`<head>`), строки `--font-display`/`--font-body` в `design/tokens.css` и `tokens.json`, `public/fonts/manrope-800-*`. Не трогаю компоненты страниц и `COURSE-DETAILS-001`/`LEDGER-SYNC-001` |
