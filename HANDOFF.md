...
| LEADS-DURABILITY-001 | IN_PROGRESS | Codex GPT-5 `/root` | `ai2/leads-durability`; worktree `C:\Users\Dias\Documents\ChatGPT\ashyq-leads-durability` | started 2026-09-13; решение по DoD без ожидания выбора БД (решение ниже фиксирую в HANDOFF как предложение к `DEPLOY-001`): заявка сначала атомарно пишется в JSONL (write-ahead), повторная отправка идемпотентна по `dedupeKey` = sha256(runId|kind|phone|name) с окном 24ч; доставка Telegram/webhook — через outbox-ledger `.data/lead-deliveries.jsonl` со статусами sent/failed, попытками и таймаутом 10с; авто-retry при чтении CRM-снапшота по backoff (чистая политика `isRetryDue` в `crm.ts`), ручной retry кнопкой в CRM (`PATCH /api/crm action:retry-delivery`); ежедневный backup `leads.jsonl` в `.data/backups/` (хранить 14); CSV-экспорт уже есть. Смена JSONL→БД остаётся за `DEPLOY-001`. Owned: `src/lib/lead-server.ts`, новый `src/lib/lead-delivery.ts`, `src/lib/crm.ts` (только добавления типов/политики), `src/lib/crm-server.ts`, `src/app/api/lead/route.ts`, `src/app/api/crm/route.ts`, `src/components/CrmDashboard.tsx` (только блок доставки), точечно `scripts/crm-check.ts`, `scripts/e2e-check.ts` (свой блок), `.github/workflows/ci.yml` (env webhook для ретрая), `HANDOFF.md`, `PRODUCT_BACKLOG.md`. Не трогаю диагностику/state, LMS, файлы V3-TOKEN-AUDIT-002 и A11Y-PERF-001 (их точки в `ci.yml` не переписываю) |
