...
| SITE-AUDIT-001 | IN_PROGRESS | Codex GPT-5 `/root` (координатор, 5 субагентов) | ветки `ai2/audit-sweep`, `ai2/audit-ux`, `ai2/epics`; worktrees `ashyq-audit-sweep` / `ashyq-audit-ux` / `ashyq-epics` | started 2026-09-13; полный аудит сайта + UX-обзор + `docs/epics.md` (Jira-концепт). Распределение: sweep — технический обход всех маршрутов (статусы, консоль, битые ссылки, titles/meta, sitemap/robots), отчёт `docs/audit/technical-2026-09-13.md`; ux — скриншоты ключевых страниц 1440/390 и обзор удобства, отчёт `docs/audit/ux-2026-09-13.md`; epics — `docs/epics.md` из backlog+HANDOFF+находок аудита. Read-only для продуктового кода; tmp-скрипты в worktree не коммитятся. Сервера на портах 3021/3022, убивают только свои PID. Субагенты HANDOFF не трогают — ветки пушат в origin, merge и закрытие делает координатор |
| CRM-ANALYTICS-001 | IN_PROGRESS | Codex GPT-5 `/root` (координатор, 2 субагента) | ветки `ai2/crm-analytics` (реализация), `ai2/crm-analytics-e2e` (e2e); worktrees `ashyq-crm-analytics` / `ashyq-crm-analytics-e2e` | started 2026-09-13; продуктовая аналитика в CRM по мотивам OpenCRM (операторские метрики, без внешних провайдеров): контракта снапшота — `stats.analytics = { leadsLast7Days: number, funnel: { diagnostics, whatsapp, contacts, season } (по записям, где был лид такого kind), bySource: Array<{ source, leads, contacts, season }> (по utm_source, desc), last14Days: Array<{ date: 'YYYY-MM-DD', leads: number }> (14 дней от сегодня, UTC, пропуски = 0) }`; расчёт в чистом `buildCrmSnapshot`, UI-секция «Аналитика» в CrmDashboard (воронка с конверсией, CSS-бары 14 дней, топ источников). Реализация (crm.ts, CrmDashboard.tsx, crm-check.ts) — ветка crm-analytics; e2e-проверки по контракту (scripts/e2e-check.ts) — ветка crm-analytics-e2e, после пуша реализации ребазится на неё. Порты 3023/3024. HANDOFF закрывает координатор после merge обеих веток |
