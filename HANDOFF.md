...
|---|---|---|---|---|
| CI-GATES-002 | IN_PROGRESS | Claude Opus 5 | ветка `claude/ci-gates` (PR в `main`); worktree `.claude/worktrees/ashyq-diagnostic-handoff-f946ca` | started 2026-09-14; подключить к CI проверки, которые сейчас гоняются только локально: `scripts/lms-e2e.ts`, `scripts/season-e2e.ts`, `scripts/token-audit.ts`, `scripts/lms-unit-check.ts`, `scripts/season-unit-check.ts`; npm-скрипты для них; лимит job 30 → 40 минут; описание новых gates в `docs/CI.md`. Owned: `.github/workflows/ci.yml`, `package.json` (только scripts), `docs/CI.md`; сами скрипты проверок — только если этого требует CI-окружение. Не трогаю продуктовый код и `COURSE-DETAILS-001`/`LEDGER-SYNC-001` |
| FONT-CLS-001 | IN_PROGRESS | Claude Opus 5 | ветка `claude/font-cls` (PR в `main`); worktree `.claude/worktrees/ashyq-diagnostic-handoff-f946ca` | started 2026-09-14; убрать CLS от подмены шрифтов (Manrope 800 в hero `/season` ≈0.18): fallback-`@font-face` с `size-adjust`/`ascent-override` для Manrope и Inter, при необходимости preload. Причина найдена: `max-width` заголовков в `ch` (ширина «0» у Manrope на ~20% больше, чем у fallback) — перевожу на `em` с тем же видом. Owned: `src/app/fonts.css`, `src/app/layout.tsx` (только шрифты/`<head>`), строки `--font-display`/`--font-body` в `design/tokens.css`, `public/fonts/manrope-800-*`; точечно строки `max-width` заголовков в `SeasonV3`/`HomeV3`/`BlogV3`/`AboutV3`/`ContactsV3.module.css`. Не трогаю `CoursesV3.module.css` (COURSE-DETAILS-001) и `LEDGER-SYNC-001` |
| DEPLOY-PREP-001 | IN_PROGRESS | Claude Opus 5 | ветка `claude/deploy-prep` (PR в `main`); worktree `.claude/worktrees/ashyq-diagnostic-handoff-f946ca` | started 2026-09-14; подготовка к деплою без выбора хостинга: `GET /api/health`, проверка env при старте прод-сервера (понятная ошибка вместо тихой поломки CRM/лидов), `output: 'standalone'` + `Dockerfile`/`.dockerignore`, `docs/DEPLOY.md` — чек-лист с плейсхолдерами «заменить перед деплоем». Owned: `src/app/api/health/**`, `src/instrumentation.ts`, `src/lib/env.ts`, `next.config.mjs`, `Dockerfile`, `.dockerignore`, `.env.example`, `docs/DEPLOY.md`. Не трогаю продуктовые экраны, CI (CI-GATES-002) и `COURSE-DETAILS-001`/`LEDGER-SYNC-001` |
| BLOG-DRAFTS-001 | IN_PROGRESS | Claude Opus 5 | ветка `claude/blog-drafts` (PR в `main`); worktree `.claude/worktrees/ashyq-diagnostic-handoff-f946ca` | started 2026-09-14; вместо заглушки «Полный текст статьи появится позже» — черновые тексты 4 демо-статей (методика IELTS/SAT по официальным форматам, Match Day по правилам демо-сезона), автор «Команда ASHYQ», `BLOG_IS_DEMO` и демо-плашка остаются до настоящих статей пользователя. Без выдуманных цифр, отзывов и результатов. Owned: `src/data/blog.ts`, строка `body` в `defaultPosts()` `src/lib/lms/local-repos.ts` |
