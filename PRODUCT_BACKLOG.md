...
| ID | Статус | Задача | Definition of done | Зависимости / границы |
|---|---|---|---|---|
| COURSE-DETAILS-001 | IN_PROGRESS | Отдельные страницы IELTS и SAT | Для каждого курса есть программа, формат, длительность, расписание/статус набора, преподаватель, стоимость или честное «по запросу», FAQ и отдельная заявка | Codex GPT-5 `/root`; временные данные явно маркировать и заменить после подтверждения |
