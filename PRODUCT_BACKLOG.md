...
| SUPABASE-FOUNDATION-001 | IN_PROGRESS | Безопасный фундамент Supabase | В репозитории есть воспроизводимые миграции профилей, LMS и чемпионата, RLS/явные grants, безопасный публичный рейтинг, локальные SQL/static checks и инструкция rollout/rollback | Не переключает runtime; Docker и auth/product decisions обязательны до adapter cutover |
