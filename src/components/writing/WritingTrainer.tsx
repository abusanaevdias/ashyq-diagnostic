      ...
      <div className={styles.layout}>
        <main className={styles.main}>
          ...
          {session.stage === 'report' && <section id="exercise" className={styles.workCard} aria-labelledby="report-title">
            ...
            {(unverifiedGrammar.length > 0 || unverifiedRevisions.length > 0) && <section className={styles.unverified}><h3>Твои неприменённые варианты</h3><p className={styles.hint}>Они не меняют ориентир автоматически. Покажи их учителю, чтобы обсудить качество текста.</p>{unverifiedGrammar.map(([sentenceId, draft]) => <div key={sentenceId}><strong>Грамматика · {grammarAttemptStatus(essay, session, sentenceId) === 'unnecessary' ? 'правка без размеченной ошибки' : 'другой вариант'}</strong><p lang="en">{draft}</p></div>)}{unverifiedRevisions.map((criterion) => <div key={criterion}><strong>{criterionLabels[criterion]}</strong><p lang="en">{session.revisionDrafts[criterion]}</p></div>)}</section>}
